...
def authenticate():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            # Uses localhost redirect, which matches the credentials.json "redirect_uris"
            creds = flow.run_local_server(
                port=0,  # 0 lets it pick an available port (no need for 5000)
                prompt='consent',
                authorization_prompt_message='Please visit this URL: {url}',
                success_message='Authentication complete. You may close this window.',
                open_browser=True  # will try, or just print URL if fails
            )
...
def fetch_new_emails(gmail_service, sheets_service, spreadsheet_id):
    ...
    try:
        ...
        for msg in messages:
            ...
            # Timestamp of when email is processed
